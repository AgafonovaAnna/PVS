...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        if (rank == 0)
            printf("Использование: mpirun -np <процессы> %s <rows> <cols> <запуски>\n", argv[0]);
        MPI_Finalize();
        return 1;
    }
    ...
    if (rows <= 0 || cols <= 0 || runs <= 0) {
        if (rank == 0)
            printf("Ошибка: все параметры должны быть больше 0\n");
        MPI_Finalize();
        return 1;
    }
    ...
    if (SIZE % nproc != 0) {
        if (rank == 0)
            printf("Ошибка: размер массива %d (rows=%d, cols=%d) не делится на %d процессов\n",
                SIZE, rows, cols, nproc);
        MPI_Finalize();
        return 1;
    }
    ...
    if (rank == 0) {
        printf("Average execution time of operation per %d runs (sec):\n", runs);
